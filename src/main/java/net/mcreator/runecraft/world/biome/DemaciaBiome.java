...
import net.mcreator.runecraft.block.PetricitelogBlock;
...
public class DemaciaBiome extends RunecraftModElements.ModElement {
	@ObjectHolder("runecraft:demacia")
	public static final CustomBiome biome = null;
	public DemaciaBiome(RunecraftModElements instance) {
		super(instance, 20);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(PetricitelogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(PetricitelogBlock.block.getDefaultState()))).baseHeight(8)
			...
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(EntityType.BEE, 15, 1, 5));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == PetricitelogBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, PetricitelogBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, PetricitelogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == PetricitelogBlock.block.getDefaultState().getBlock()
					|| blockType == PetricitelogBlock.block.getDefaultState().getBlock()
