...
package net.mcreator.runecraft.world.dimension;
...
public class RuneterraDimension extends RunecraftModElements.ModElement {
	@ObjectHolder("runecraft:runeterra")
	public static final ModDimension dimension = null;
	@ObjectHolder("runecraft:runeterra_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public RuneterraDimension(RunecraftModElements instance) {
		super(instance, 1);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("runeterra"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("runecraft:runeterra")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("runecraft:runeterra"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("runecraft:runeterra"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("ocean")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("runecraft:ionia")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("runecraft:noxus")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("runecraft:demacia")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("runecraft:shurima")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("runecraft:frejord")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("runecraft:ixtal")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new RuneterraItem().setRegistryName("runeterra"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("runeterra_portal");
		}
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = RuneterraDimension.CustomPortalBlock.createPatternHelper(entity.world, new BlockPos(pos));
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == Blocks.SMOOTH_STONE.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == Blocks.SMOOTH_STONE.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == Blocks.SMOOTH_STONE.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "runeterra_portal",
	...
	public static class TeleporterDimensionMod extends Teleporter {
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? Blocks.SMOOTH_STONE.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.SMOOTH_STONE.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = RuneterraDimension.CustomPortalBlock.createPatternHelper(this.world,
						blockpos);
